...
**Layer F (Live-fire drill):** The cultivation-os dev server was not available in the test environment. Layer F evidence is deferred pending operator availability of the cultivation-os stack. See risk R6 in plan §15: if server unavailable, AskUserQuestion before claiming Class A for HTTP boundary. The tests cover all HTTP behavior via mocked `post_pending_task` and `urllib.request.urlopen`. The real-SQLite integration tests (Layer B) exercise the `_verdicts_store.py` substrate against actual SQLite files.
...
| Coverage floor | `_verdicts_store.py`: 100%, `verdicts_workflow.py`: 88% | Both exceed 85% floor from plan §12 Layer D |
...
- Layer F live-fire drill (plan §12.F): cultivation-os dev server not available in this environment. HTTP boundary tested via mocked `urllib.request.urlopen` (Layer A synthetic). Operator must run Layer F drill before H2 merge judgment if live-fire Class A evidence is required.
