...
Change 'fix-p1c-draftflow-absent-impl': 5 aiv commits + 1 plain commit (bug catalogs). All 28 verdicts tests PASS. mypy clean on production code. No machine-measured line-coverage artifact exists (`pytest --cov` was not run); behavioral coverage is evidenced by 29 verdicts tests PASS and all 6 bug contracts resolved (see Class D). RED tests in test_verdicts_flow.py (8) → all GREEN. P1c-draftflow-absent defect is addressed.
