...
5. 29 tests covering all 15 verification-matrix items: Card clamping, kebab normalization, watermark idempotency, reconcile gate (done/pending/deferred), partial-failure skip, D4 filter-by-stored-IDs, real SQLite persistence across restart, anti-regression imports
...
Verdicts-specific tests: 29 tests (test_verdicts_flow.py: 8, test_verdicts_workflow.py: 21) — all PASSED.
...
Change 'fix-p1c-draftflow-absent-impl': 5 aiv commits + 1 plain commit (bug catalogs). All 29 verdicts tests PASS (test_verdicts_flow.py: 8, test_verdicts_workflow.py: 21). mypy clean on production code. No machine-measured line-coverage artifact exists (`pytest --cov` was not run); behavioral coverage is evidenced by 29 verdicts tests PASS and all 6 bug contracts resolved (see Class D). RED tests in test_verdicts_flow.py (8) → all GREEN. P1c-draftflow-absent defect is addressed.
